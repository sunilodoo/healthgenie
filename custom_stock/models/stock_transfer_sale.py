...
class StockTranferSale(models.Model):
	_name = 'stock.transfer.sale'
	_description = 'Stock Tranfer Sale'
	# _order = 'sequence'
	# _order = "name desc"
	_sql_constraints = [
	...
	gstin = fields.Char(related="ship_to_fc.gstin", string="GSTIN(Receiver)", store=True)
	state_id = fields.Many2one(related="ship_to_fc.state_id", string="State(Receiver)", store=True)
	...
	delivery_challan = fields.Boolean(string="Delivary Challan", default=False, compute='comp_delivery_challan', store=True)
	...
	stock_transfer_line = fields.One2many('stock.transfer.sale.line', 'stock_transfer_sale_id', string='Stock Tranfer Sale Lines', copy=True, auto_join=True)
	company_id = fields.Many2one(related='ship_to_fc.company_id', string='Company', store=True, readonly=True, index=True)
	...
	total_invoice_vales = fields.Monetary(string="Total Invoice Value", store=True, readonly=True, compute='_amount_all')
	# sgst_rate = fields.Float(string="Sgst Rate")
	# sgst_amount = fields.Monetary(string="Sgst Amount")
	# utgst_rate = fields.Float(string="Utgst Rate")
	...
	# comp_c_r = fields.Float(string="Compensatory Cess Rate")
	# comp_c_a = fields.Monetary(string="Compensatory Cess Amount")
	# gstin_of_receiver = fields.Many2one('stock.warehouse', string="Warehouse")
	# trn_type = fields.Selection([('fcr', 'FC_REMOVAL'), ('fcrc', 'FC_REMOVAL-Cancel'), ('fct', 'FC_TRANSFER')], string="Transaction Type")
	...
	def create(self, vals):
		# print("-----------------------create------1--------------")
		res = super(StockTranferSale, self).create(vals)
		if res.ship_from_fc:
			new_seq = '23-24'+'/'+res.ship_from_fc.code+'/'+str(res.ship_from_fc.next_number_s_t_s).zfill(5)
			# new_seq = '21-22'+'/'+res.ship_from_fc.code+'/'+str(res.ship_from_fc.next_number_s_t_s).zfill(5)
			true_false = res.write({'name': new_seq})
			if true_false:
				res.ship_from_fc.write({'next_number_s_t_s': res.ship_from_fc.next_number_s_t_s+1})
		return res
	@api.depends('ship_to_fc', 'ship_from_fc')
	@api.onchange('ship_to_fc', 'ship_from_fc')
	def comp_delivery_challan(self):
		# for s_l in self.env['stock.transfer.sale.line'].search([('stock_transfer_sale_id', '=', self.id)]):
		# 	print("-----------------234------", s_l._onchange_product())
		for line in self:
			if line.ship_to_fc.state_id and line.ship_from_fc.state_id:
				if line.ship_to_fc.state_id.name == line.ship_from_fc.state_id.name:
					line.update({'delivery_challan': True})
	

class StockTranferSaleLine(models.Model):
	_name = 'stock.transfer.sale.line'
	_description = 'Stock Tranfer Sale Line'
	stock_transfer_sale_id = fields.Many2one('stock.transfer.sale', string='Order Reference', required=True, ondelete='cascade', index=True, copy=False)
	...
	currency_id = fields.Many2one(related='stock_transfer_sale_id.currency_id', string="Currency ID")
	...
	sales_ladgers = fields.Char(string="Sales Ledgers")
	...
	def create(self, vals):
		# print("-----------------------create------2--------------")
		res = super(StockTranferSaleLine, self).create(vals)
	...
	# @api.onchange('quntity', 'invoice_vales', 'stock_transfer_sale_id.ship_to_fc', 'stock_transfer_sale_id.ship_from_fc')
	def compute_amount(self):
		...
		for line in self:
			...
			for tax in line.product_product_id.taxes_id:
				...
				taxable_amount =0.0
				taxable_value = 0.0
				if line.stock_transfer_sale_id.ship_to_fc.state_id.name != line.stock_transfer_sale_id.ship_from_fc.state_id.name:
					vals['sales_ladgers'] = 'Br. Sale-IGST-'+str(int(s_l))+'%'
					# vals['sales_ladgers'] = 'Br. Sale-IGST-'+str(int(s_l))+'%' if s_l else ''
				...
				if line.stock_transfer_sale_id.ship_to_fc.state_id.name == line.stock_transfer_sale_id.ship_from_fc.state_id.name:
					vals['sales_ladgers'] = 'Br. Sale-IGST-'+str(0)+'%'
					# vals['sales_ladgers'] = 'Br. Sale-IGST-'+str(0)+'%' if s_l else ''
