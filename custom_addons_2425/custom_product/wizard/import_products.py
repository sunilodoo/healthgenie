...
class ImportProducts(models.TransientModel):
	...
	def update_sku_product(self):
		...
		count = 1
		for row in reader:
			sku_code = row[headers['SKU CODE']].strip()
			prod_pool = self.env['product.product'].search([('default_code', '=', sku_code)])
			pro_pro_id = self.env['product.template'].search([('default_code', '=', sku_code)])
			# product_type = row[headers['product type']].strip()
			categ2 = row[headers['prd Cat ']].strip(),
			sub_categ = row[headers['prd sub cat']].strip(),
			product_type = row[headers['product type']].strip(),

			product_type1 = self.env['product.type'].search([('name', '=', product_type)])

			product_catg2 = self.env['product.category2'].search([('name', '=', categ2)])
			sub_catg = self.env['product.category3'].search([('name', '=', sub_categ)])
			categ_value = product_catg2

			print(categ_value, "------------------------------")
			sub_categ_value = sub_catg

			sku_vals = {
				'model_no': row[headers['Model no.']].strip(),
				'model_name': row[headers['model name']].strip(),
				'product_specification': row[headers['product specification ']].strip(),
				'product_material': row[headers['product material']].strip(),
				'supplier_name': row[headers['supplier name']].strip(),
				'product_box_dimension_l': row[headers['prod B dim L']].strip(),
				'product_box_dimension_b': row[headers['prod B dim B']].strip(),
				'product_box_dimension_h': row[headers['prod B  dim H / t']].strip(),
				'carton': row[headers['pcs / M carton']].strip(),
				'mc_dimm_l_cms': row[headers['MC dimm L cms']].strip(),
				'type_of_product': product_type1.id,
				'categ2': categ_value.id,
				'categ3': sub_categ_value.id,
				'mc_dimm_b_cms': row[headers['MC dimm B cms ']].strip(),
				'mc_dimm_h_cms': row[headers['MC dimm H cms ']].strip(),
				'l10n_in_hsn_description': row[headers['HSN rate']].strip()
			}

			prod_pool.write(sku_vals)
			# pro_pro_id.write(sku_vals)
