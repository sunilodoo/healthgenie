	...
	def action_confirm(self):
		...
		if self.state == 'draft':
			...
			if self.name == 'New':
				...
				# self.name = '24-25/'+'RTO'+'/'+str(self.warehouse_id.rto_b2b).zfill(4)
				# # print(str(self.warehouse_id.rto_b2c).zfill(4),"======123=========",self.name)
				#
				#
				...
				#sequence_number = self.env['ir.sequence'].next_by_code('return.order') or 'New'
				self.name = '2425/' + 'RTO' + str(self.warehouse_id.state_id.code) + '/' + str(self.warehouse_id.state_id.rto_b2b).zfill(5)

				if self.name:
					wh_so_seq = self.warehouse_id.state_id.write({'rto_b2b': self.warehouse_id.state_id.rto_b2b + 1})

			# if self.sale_number_b2b:
			# 	wh_so = self.warehouse_id.write({'rto_b2b': self.warehouse_id.rto_b2b+1})
			self.product_cat = self.product_product_id.categ2.name
			self.state = 'done'
