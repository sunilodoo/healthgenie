        ...
        if invoice_ids:
            ...
            for inv in invoice_ids:
                ...
                for inv_l in inv.invoice_line_ids:
                    ...
                    ws.write(row, 17,
                             str(inv.partner_id.state_id.l10n_in_tin) + '-' + str(inv.partner_id.state_id.name))
                    ...
                    ws.write(row, 24, 'CGST+SGST Sale-' + 'B2C' + '-' + str(int(inv_l.tax_ids[
                                                                                    0].amount)) + '%' if inv.partner_id.state_id.name == inv.warehouse_id.state_id.name else 'IGST Sale-' + 'B2C' + '-' + str(
                        int(inv_l.tax_ids[0].amount)) + '%')
                    # ws.write(row, 24, 'B2C-Output Tax CGST' if inv.partner_id.state_id.name == inv.warehouse_id.state_id.name else 'B2C-Output Tax IGST')
                    ...
                    ws.write(row, 26,
                             'B2C-Output Tax CGST' if inv.partner_id.state_id.name == inv.warehouse_id.state_id.name else '')
                    ws.write(row, 27, inv_l.sgst_amount)
                    ws.write(row, 28,
                             'B2C-Output Tax SGST ' if inv.partner_id.state_id.name == inv.warehouse_id.state_id.name else '')
                    ...
                    ws.write(row, 30,
                             'B2C-Output Tax IGST' if inv.partner_id.state_id.name != inv.warehouse_id.state_id.name else '')
                    ...
                    ws.write(row, 43, '')
                    ws.write(row, 44, 'Being order id ' + str(inv.sale_number) + ' Invoice no ' + str(
                        inv.invoice_number) + ' suborder id ' + str(inv.order_id) + ' sale through ' + str(
                        inv.order_type) + ' customer name ' + str(inv.partner_id.name) + ' state ' + str(
                        inv.partner_id.state_id.name) + ' Godown ' + str(inv.warehouse_id.name))
                    ws.write(row, 45, 'NEW INVOICE')
                    ws.write(row, 46,
                             'Sale-CGST+SGST-B2C' if inv.partner_id.state_id.name == inv.warehouse_id.state_id.name else 'Sale-IGST-B2C')
                    ws.write(row, 47, '')
